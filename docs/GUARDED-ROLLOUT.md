...
5. After containment, remove only regular, non-symlink SQLite sidecars whose WAL is exactly zero bytes. A non-empty WAL or uncertain sidecar remains a hard failure.
6. Recheck Git and database preconditions.
7. Create byte-exact SQLite backups after proving no WAL/SHM sidecars remain. Record and verify source/backup UID, GID, mode, size, canonical path, and SHA-256.
8. Run the repository's additive migrations and validate the current schema.
9. Reset failed state for every selected unit, start every service, verify active state, inspect startup error logs, and revalidate databases. The systemd journal's benign `-- No entries --` response is accepted; actual error output remains fatal.
