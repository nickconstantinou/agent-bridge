...
export class BridgeEngine {
  ...
  private _buildContextAccess(chatKey: string): { prompt: string; env: Record<string, string> } | null {
    ...
    const advisorPrompt = advisorCapability ? [
      ...
      "Consult the advisor when:",
      "- choosing architecture or implementation strategy for consequential work;",
      "- the task spans multiple modules or has unclear trade-offs;",
      "- security, authentication, billing, migration, deployment, deletion, or merge safety is involved;",
      "- the same approach has failed twice or the root cause remains uncertain;",
      "- reviewing a high-impact plan before implementation;",
      "- deciding between materially different approaches with incomplete evidence.",
      "Do not consult it for:",
      "- routine edits with an obvious implementation;",
      "- formatting, documentation-only changes, or simple factual questions;",
      "- work already covered by tests and an established repository pattern;",
      "- repeated confirmation of the same decision.",
      "Use modes:",
      "- plan: architecture and implementation approach;",
      "- review: challenge a proposed plan, patch, or conclusion;",
      "- debug: unresolved or repeated failure;",
      "- risk: security, deployment, destructive or approval-sensitive work;",
      "- decision: consequential trade-off between options.",
      "Call at most once for the same decision unless new evidence materially changes the problem.",
