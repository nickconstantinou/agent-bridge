...
const geminiCmd = GEMINI || "/home/openclaw/.nvm/versions/node/v24.14.1/bin/gemini";
const projectDir = PROJECT_DIR || "/home/openclaw/.openclaw/workspace/projects/agent-bridge";
...
async function runWithACP({ prompt, onProgress, onCancel }) {
  ...
  // Note: --acp hangs in non-interactive mode, so we skip it for testing
  const child = spawn(geminiCmd, [
    ...
    env: { ...process.env, GEMINI: geminiCmd, BRIDGE_PROJECT_DIR: projectDir }
  });
