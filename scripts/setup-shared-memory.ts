...
const antigravityInstructionsPath = join(homeDir, "ANTIGRAVITY.md");
...
const existingAntigravityInstructions = readText(antigravityInstructionsPath);
...
const nextAntigravityInstructions = renderAgentMemoryInstructionFile(existingAntigravityInstructions, "antigravity", dbPath);
...
if (!verifyOnly) {
  ...
  writeText(antigravityInstructionsPath, nextAntigravityInstructions);
  writeText(claudeInstructionsPath, nextClaudeInstructions);
  spawnSync("npx", ["tsx", "scripts/seed-agent-memory.ts"], { stdio: "inherit" });
}

const result = verifySharedMemoryConfigs({ codex: nextCodexInstructions, antigravity: nextAntigravityInstructions, claude: nextClaudeInstructions });
if (!result.ok) {
...
if (verifyOnly) {
  ...
  console.log(`shared-memory setup: wrote ${antigravityInstructionsPath}`);
