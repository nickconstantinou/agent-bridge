...
describe("systemd templates", () => {
  it("pin service-specific env files and absolute CLI paths", () => {
    ...
    expect(codex).toContain("EnvironmentFile=%h/agent-bridge/.env.codex");
    expect(gemini).toContain("EnvironmentFile=%h/agent-bridge/.env.gemini");
    expect(codex).toContain("BRIDGE_ENV_FILE=%h/agent-bridge/.env.codex");
    expect(gemini).toContain("BRIDGE_ENV_FILE=%h/agent-bridge/.env.gemini");
